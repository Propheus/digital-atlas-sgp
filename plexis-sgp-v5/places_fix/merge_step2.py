...
for scale,key in [("hex8","hex8_id"),("hex9","hex9_id")]:
    ...
    for tag in ["saturation_gap","demand_pull"]:
    ...
    print(f"{scale}: {n0}->{m.shape} | gap_pharmacy_beauty={'gap_pharmacy_beauty' in m.columns} cap_health_medical present={'cap_health_medical' in m.columns}")
