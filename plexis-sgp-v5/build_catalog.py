...
DESCRIPTIONS.update({
    ...
    # per-place micrograph (pmg_*) — the per-venue site fingerprint
    "pmg_competitors_400m": dict(description="SAME-category places within 400 m of this place", units="count", source_stage="10p"),
    "pmg_competitors_800m": dict(description="SAME-category places within 800 m", units="count", source_stage="10p"),
    "pmg_closest_competitor_m": dict(description="Distance to the nearest same-category place", units="m", source_stage="10p"),
    "pmg_competitor_rating_avg": dict(description="Mean rating of nearby competitors — incumbent quality bar", units="stars", source_stage="10p"),
    "pmg_complements_400m": dict(description="Complementary-category places within 400 m (demand context)", units="count", source_stage="10p"),
    "pmg_complements_800m": dict(description="Complementary-category places within 800 m", units="count", source_stage="10p"),
    "pmg_complement_categories_present": dict(description="Distinct complementary categories present within 400 m", units="count", source_stage="10p"),
    "pmg_complement_diversity": dict(description="Entropy of the complementary mix around the place", units="0-1", source_stage="10p"),
    "pmg_anchors_400m": dict(description="Magnet/anchor places within 400 m", units="count", source_stage="10p"),
    "pmg_anchors_800m": dict(description="Magnet/anchor places within 800 m", units="count", source_stage="10p"),
    "pmg_closest_anchor_m": dict(description="Distance to the nearest anchor place", units="m", source_stage="10p"),
    "pmg_anchor_strength_sum": dict(description="Summed anchor strength in the place's neighbourhood", units="index", source_stage="10p"),
    "pmg_walk_dist_mrt_m": dict(description="Walk distance from the place to nearest MRT/LRT", units="m", source_stage="10p"),
    "pmg_walk_dist_bus_m": dict(description="Walk distance to nearest bus stop", units="m", source_stage="10p"),
    "pmg_near_mrt_400m": dict(description="MRT within 400 m of the place", units="bool", source_stage="10p"),
    "pmg_near_bus_300m": dict(description="Bus stop within 300 m of the place", units="bool", source_stage="10p"),
    "pmg_snap_delta_m": dict(description="Geocode-to-network snap distance (QA)", units="m", source_stage="10p"),
    "pmg_hex_walk_score": dict(description="Walkability score of the place's hex (context copy)", units="0-1", source_stage="10p"),
    "pmg_hex_transit_score": dict(description="Transit score of the place's hex (context copy)", units="0-1", source_stage="10p"),
})

# S11 mobility pack — generated curated entries
_ANCHOR_LBL = {"cbd": "CBD (Raffles Place)", "orchard": "Orchard", "jurong_east":
               "Jurong East", "one_north": "one-north", "changi_business":
               "Changi Business Park", "tampines_hub": "Tampines Hub", "nus": "NUS",
               "ntu": "NTU", "sgh": "SGH", "cgh": "CGH", "kkh": "KKH", "ttsh": "TTSH"}
for _a, _l in _ANCHOR_LBL.items():
    DESCRIPTIONS[f"time_to_{_a}_min"] = dict(
        description=f"Door-to-door transit travel time to {_l} (mobility-v2 reach model)",
        units="min", source_stage="S11")
for _f, _d in {
    "accessibility": "composite access", "connectivity": "network connectivity",
    "distance": "distance to transit", "last_mile": "last-mile friction",
    "line_pressure": "line crowding pressure", "low_frequency": "service frequency shortfall",
    "reach_gap": "destination reach shortfall", "children_gap": "child-population service gap",
    "elderly_gap": "elderly service gap", "dorm_gap": "dorm-worker service gap",
    "fdw_gap": "FDW service gap", "low_income_gap": "low-income service gap"}.items():
    DESCRIPTIONS[f"adq_f_{_f}"] = dict(
        description=f"Adequacy v3 factor score: {_d} (mobility-v2 model)",
        units="0-100", source_stage="S11")
DESCRIPTIONS.update({
    "n_dest_reachable": dict(description="Key destinations reachable by transit (mobility-v2)", units="count", source_stage="S11"),
    "n_dest_within_45min": dict(description="Key destinations within 45-min transit", units="count", source_stage="S11"),
    "pct_dest_within_45min": dict(description="Share of key destinations within 45 min", units="ratio", source_stage="S11"),
    "pct_dest_within_60min": dict(description="Share of key destinations within 60 min", units="ratio", source_stage="S11"),
    "n_lines_to_cbd": dict(description="Distinct rail lines connecting toward the CBD", units="count", source_stage="S11"),
    "n_stations_walking": dict(description="Stations within walking reach", units="count", source_stage="S11"),
    "mrt_reach_eff_min": dict(description="Effective door-to-MRT time incl. feeder + wait (multi-leg model)", units="min", source_stage="S11"),
    "mrt_reach_walk_min": dict(description="Walk leg of MRT reach", units="min", source_stage="S11"),
    "mrt_reach_bus_min": dict(description="Feeder-bus leg of MRT reach", units="min", source_stage="S11"),
    "mrt_reach_bus_wait_min": dict(description="Feeder wait of MRT reach", units="min", source_stage="S11"),
    "mrt_reach_crowd": dict(description="Crowding multiplier on the reach path", units="index", source_stage="S11"),
    "mrt_reach_index": dict(description="Composite MRT reach quality", units="0-1", source_stage="S11"),
    "mrt_reach_n_feeders": dict(description="Feeder bus services to nearest MRT", units="count", source_stage="S11"),
    "mrt_reach_mode": dict(description="Reach mode: walk / feeder / poor", units="category", source_stage="S11"),
    "peak_wait_min": dict(description="Expected peak-hour wait (best mode)", units="min", source_stage="S11"),
    "peak_wait_bus_only_min": dict(description="Peak wait, bus only", units="min", source_stage="S11"),
    "peak_wait_mrt_only_min": dict(description="Peak wait, MRT only", units="min", source_stage="S11"),
    "crowding_load_factor": dict(description="Peak load factor on serving lines", units="index", source_stage="S11"),
    "adq_default": dict(description="Transport adequacy v3 (default profile, 0-100)", units="0-100", source_stage="S11"),
    "adq_core": dict(description="Adequacy core (pre-vulnerability)", units="0-100", source_stage="S11"),
    "adq_v2": dict(description="Adequacy v2 (availability-floored legacy)", units="0-100", source_stage="S11"),
    "adq_default_elderly": dict(description="Adequacy, elderly profile", units="0-100", source_stage="S11"),
    "adq_default_family": dict(description="Adequacy, family profile", units="0-100", source_stage="S11"),
    "adq_default_workers": dict(description="Adequacy, workers profile", units="0-100", source_stage="S11"),
    "adq_core_elderly": dict(description="Adequacy core, elderly", units="0-100", source_stage="S11"),
    "adq_core_family": dict(description="Adequacy core, family", units="0-100", source_stage="S11"),
    "adq_core_workers": dict(description="Adequacy core, workers", units="0-100", source_stage="S11"),
    "adq_gap_default": dict(description="Adequacy gap (default profile)", units="0-100", source_stage="S11"),
    "adq_gap_core": dict(description="Adequacy gap (core)", units="0-100", source_stage="S11"),
    "adq_gap_equity_max": dict(description="Worst per-profile equity gap", units="0-100", source_stage="S11"),
    "adq_availability_v2": dict(description="Transit availability composite", units="0-100", source_stage="S11"),
    "adq_worst_factor_value": dict(description="Score of the worst adequacy factor", units="0-100", source_stage="S11"),
    "adq_worst_factor": dict(description="Name of the worst adequacy factor", units="category", source_stage="S11"),
    "adq_primary_factor": dict(description="Primary driving factor (default profile)", units="category", source_stage="S11"),
    "adq_primary_gap_reason": dict(description="Primary gap explanation tag", units="category", source_stage="S11"),
    "min15_score": dict(description="15-minute-city score (calibrated: Toa Payoh 100 / Lim Chu Kang 13)", units="0-100", source_stage="S11"),
    "min15_essentials": dict(description="15-min subscore: daily essentials", units="0-100", source_stage="S11"),
    "min15_health": dict(description="15-min subscore: health", units="0-100", source_stage="S11"),
    "min15_retail": dict(description="15-min subscore: retail", units="0-100", source_stage="S11"),
    "min15_school": dict(description="15-min subscore: schools", units="0-100", source_stage="S11"),
    "min15_count_essentials": dict(description="Essential amenities within 15 min", units="count", source_stage="S11"),
    "min15_count_health": dict(description="Health amenities within 15 min", units="count", source_stage="S11"),
    "min15_count_retail": dict(description="Retail within 15 min", units="count", source_stage="S11"),
    "min15_count_school": dict(description="Schools within 15 min", units="count", source_stage="S11"),
    "min15_nearest_clinic_m": dict(description="Nearest clinic (15-min model)", units="m", source_stage="S11"),
    "min15_nearest_hawker_m": dict(description="Nearest hawker centre", units="m", source_stage="S11"),
    "min15_nearest_park_m": dict(description="Nearest park", units="m", source_stage="S11"),
    "min15_nearest_school_m": dict(description="Nearest school", units="m", source_stage="S11"),
    "min15_nearest_super_m": dict(description="Nearest supermarket", units="m", source_stage="S11"),
    "pop_resident_citizen": dict(description="Resident citizens", units="persons", source_stage="S11"),
    "pop_resident_pr": dict(description="Permanent residents", units="persons", source_stage="S11"),
    "pop_nr_ep": dict(description="Employment-pass holders", units="persons", source_stage="S11"),
    "pop_nr_fdw": dict(description="Foreign domestic workers", units="persons", source_stage="S11"),
    "pop_nr_sp": dict(description="S-pass holders", units="persons", source_stage="S11"),
    "pop_nr_wp_other": dict(description="Other work-permit holders (non-dorm)", units="persons", source_stage="S11"),
    "low_income_pop": dict(description="Estimated low-income residents", units="persons", source_stage="S11"),
    "walking_dependent_count": dict(description="Walking-dependent residents (no car/PT-captive)", units="persons", source_stage="S11"),
    "vulnerability_share": dict(description="Vulnerable-population share (adequacy v3 multiplier input)", units="ratio", source_stage="S11"),
    "vulnerability_penalty": dict(description="Adequacy penalty from vulnerability double-threshold", units="points", source_stage="S11"),
    "access_vuln_share": dict(description="Access-vulnerable share", units="ratio", source_stage="S11"),
    "access_vuln_penalty": dict(description="Access-vulnerability penalty", units="points", source_stage="S11"),
    "crowd_sensitive_share": dict(description="Crowding-sensitive share", units="ratio", source_stage="S11"),
    "crowd_equity_penalty": dict(description="Crowding equity penalty", units="points", source_stage="S11"),
    "ped_crossings_count": dict(description="Pedestrian crossings in hex", units="count", source_stage="S11"),
    "ped_greenman_count": dict(description="Green Man+ (extended-time) crossings", units="count", source_stage="S11"),
    "lrt_stations": dict(description="LRT stations in hex", units="count", source_stage="S11"),
    "lrt_stations_in_500m": dict(description="LRT stations within 500 m", units="count", source_stage="S11"),
    "dist_to_nearest_lrt_m": dict(description="Distance to nearest LRT station", units="m", source_stage="S11"),
    "bus_stops_in_400m": dict(description="Bus stops within 400 m of centroid", units="count", source_stage="S11"),
    "bus_stops_in_800m": dict(description="Bus stops within 800 m", units="count", source_stage="S11"),
    "mrt_stations_in_500m": dict(description="MRT stations within 500 m", units="count", source_stage="S11"),
    "mrt_stations_in_1km": dict(description="MRT stations within 1 km", units="count", source_stage="S11"),
    "nearest_mrt_st_peak_taps": dict(description="Peak taps at the nearest MRT station", units="taps", source_stage="S11"),
    "last_mile_friction": dict(description="Last-mile friction composite", units="index", source_stage="S11"),
    "multimodal_score": dict(description="Multi-modal option richness", units="0-1", source_stage="S11"),
    "transit_mode_count": dict(description="Distinct transit modes serving hex", units="count", source_stage="S11"),
    "pc_cat_pharmacy_beauty": dict(description="Count of retail pharmacy / health-&-beauty places (Watsons, Guardian, Unity, independent pharmacies) — nous-consolidated split OUT of health_medical so their demand is modeled as retail footfall, not clinic catchment.", units="count", source_stage="V6"),
    "cap_pharmacy_beauty": dict(description="Huff capture potential for retail pharmacy / H&B (lambda 700 neighborhood-retail). Tracks shopping catchment (corr retail_footfall 0.74) not clinic (corr pull_hospital 0.22). nous-consolidated.", units="outlets-equiv", source_stage="V6"),
    "gap_pharmacy_beauty": dict(description="Supply gap for retail pharmacy / H&B = (national_avg - local saturation)/national_avg. nous-consolidated.", units="ratio", source_stage="V6"),
    "gap_convenience": dict(description="Supply gap for convenience = (national_avg - local saturation)/national_avg, off the DEPOLLUTED convenience set (2,164, not the old 5,762 with ATMs/vending). nous-consolidated.", units="ratio", source_stage="V6"),
    ...
    "rent_retail_locality_obs_psf": dict(description="OBSERVED URA median retail rent for the cell's locality ($psf/mo, latest qtr) — the defensible ABSOLUTE anchor (Orchard $8.59 / Central-ex-Orchard $4.59 / Outside-Central $5.15). Use this for absolute figures; use rent_retail_psf_med for within-grid ranking. (nous V5/A3).", units="$psf/mo", source_stage="V5"),
    ...
    "zone_type": dict(description="URA zone type of the hex (PA→SZ→hex8 propagated)", units="category", source_stage="S11"),
    "zone_type_broad": dict(description="Broad zone class (residential/industrial/airport/nature/islands/future) — the NA-masking rule", units="category", source_stage="S11"),
    "linkway_len_m": dict(description="Covered-linkway length in hex (7,012-segment LTA layer) — sheltered-walk density", units="m", source_stage="S11"),
    "cycling_path_len_m": dict(description="Cycling-path length in hex", units="m", source_stage="S11"),
    "linkway_per_road_km": dict(description="Covered-linkway metres per road km — shelter coverage ratio", units="m/km", source_stage="S11"),
    "pr_share": dict(description="PR share of resident population (citizen/PR ratio signal; levels deduped away)", units="ratio", source_stage="S11"),
    "low_income_share": dict(description="Low-income share of residents (level deduped vs pop_hdb; share is the signal)", units="ratio", source_stage="S11"),
})
...
DATASETS = [
    ...
    ("hex/hex8_mobility_pack.parquet",   "hex8",   "S11 mobility pack: travel-time anchors, destination reach, MRT effective-reach, waits/crowding, adequacy v3 (adq_*), 15-min city, pop pass-type splits, vulnerability, linkways/cycling", "S11"),
    # Master bundles
    ...
    ("places/sgp_places_micrograph.parquet", "place", "Per-place micrograph (pmg_*): 400/800m competitors, complements, anchors, transit walk context — the per-venue site fingerprint", "10p"),
]
